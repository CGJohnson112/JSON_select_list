...
fetch(url)
  ...
  .then(function(data) {
    ...
    function AtoZ() {
      ...
      var AtoZ = data.map(user => {return `<div class="card"><p>Name: ${user.name}</p><p>Rating: ${user.rating}</p><hr/></div>`}).join('');
      document.querySelector('#myData').insertAdjacentHTML("afterbegin", AtoZ);
      data.splice(0, data.length);
    }

    function ZtoA() {
      var ZtoA = data.sort((b,a) => (a.name > b.name) ? 1 : -1);
      ZtoA = data.map(user => {return `<div class="card"><p>Name: ${user.name}</p><p>Rating: ${user.rating}</p><hr/></div>`}).join('');
